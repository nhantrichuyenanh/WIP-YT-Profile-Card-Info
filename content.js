(() => {
    ...
    const GENERAL_BACKGROUND = 'var(--yt-spec-general-background-a)';
    ...
    function createInfoBox(icon, text) {
        ...
        box.innerHTML = icon + ' ' + text; // TODO: Unsafe assignment to innerHTML
        styleInfoBox(box);
        return box;
    }
    ...
    function createBadgeBox(icon, badgeTitle, badgeSubtitle) {
        ...
        iconSpan.innerHTML = icon; // TODO: Unsafe assignment to innerHTML
    ...
    function createRecentCommentsBox(label, commentEntries) {
        ...
        const box = document.createElement('div');
        box.className = 'yt-enhanced-info-item';
        box.innerHTML = '🗨️ ' + label; // TODO: Unsafe assignment to innerHTML
        styleInfoBox(box);
        ...
        box.addEventListener('click', (e) => {
            ...
            function onClickOutside(ev) {
                if (!wrapper.contains(ev.target) && !currentDropdown.contains(ev.target)) {
                    closeDropdown();
    ...
    function createSubscriptionsBox(label, subEntries) {
        ...
        const box = document.createElement('div');
        box.className = 'yt-enhanced-info-item';
        box.innerHTML = '🔔 ' + label; // TODO: Unsafe assignment to innerHTML
        styleInfoBox(box);
        wrapper.appendChild(box);
    ...
    function extractProfileInfo() {
        ...
        if (badgeSection) {
            badgeSection.querySelectorAll('.profileBadgeViewModelHost').forEach(badge => {
                const badgeInfo = { title: null, subtitle: null, icon: null };

                const badgeTitleElem = badge.querySelector('.profileBadgeViewModelBadgeDescription');
                if (badgeTitleElem) badgeInfo.title = badgeTitleElem.textContent.trim();

                const badgeSubtitleElem = badge.querySelector('.profileBadgeViewModelBadgeSubtitle');
                if (badgeSubtitleElem) badgeInfo.subtitle = badgeSubtitleElem.textContent.trim();
        ...
        // recent comments
        const allHeaderSections = profileCard.querySelectorAll('.ytProfileInfoViewModelHeader');
        for (const headerSection of allHeaderSections) {
            const parentActivity = headerSection.closest('.ytProfileInfoViewModelActivity');
            if (parentActivity && parentActivity.querySelector('.ytCommentInteractionViewModelFrame')) {
                const headerTitle = headerSection.querySelector('.ytProfileInfoViewModelSectionTitle');
                if (headerTitle) {
                    info.recentCommentsLabel = headerTitle.textContent.trim();
                    console.log("[WIP] YouTube Profile Card Info: Recent comments label found", info.recentCommentsLabel);
                }
                break;
            }
        }

        profileCard.querySelectorAll('.ytCommentInteractionViewModelFrame').forEach(frame => {
            const videoTitleElem = frame.querySelector('.ytCommentInteractionViewModelVideoTitle');
            const commentContentElem = frame.querySelector('.ytCommentInteractionViewModelCommentContent');

            // title is the last text node with length > 1 (skips Commented on ")
            let videoTitle = null;
            if (videoTitleElem) {
                const textNodes = Array.from(videoTitleElem.childNodes)
                    .filter(n => n.nodeType === Node.TEXT_NODE && n.textContent.trim().length > 1);
                videoTitle = textNodes.length > 0 ? textNodes[textNodes.length - 1].textContent.trim() : null;
            }

            const commentContent = commentContentElem ? commentContentElem.textContent.trim() : null;
            if (videoTitle || commentContent) info.recentComments.push({ videoTitle, commentContent });
        });
        console.log("[WIP] YouTube Profile Card Info: Recent comments scraped", info.recentComments);

        // subscriptions
        const subHeaderSection = profileCard.querySelector('.ytProfileInfoViewModelSubHeader');
        if (subHeaderSection) {
            const subHeaderTitle = subHeaderSection.querySelector('.ytProfileInfoViewModelSectionTitle');
            if (subHeaderTitle) {
                info.subscriptionsLabel = subHeaderTitle.textContent.trim();
                console.log("[WIP] YouTube Profile Card Info: Subscriptions label found", info.subscriptionsLabel);
            }
        }

        profileCard.querySelectorAll('.ytProfileInfoViewModelSub').forEach(sub => {
            const imgElem = sub.querySelector('avatar-view-model img');
            const nameElem = sub.querySelector('.ytSharedSubscriptionViewModelChannelName');
            const avatarSrc = imgElem ? imgElem.src : null;
            const channelName = nameElem ? nameElem.textContent.trim() : null;
            if (avatarSrc || channelName) info.subscriptions.push({ avatarSrc, channelName });
        });
        console.log("[WIP] YouTube Profile Card Info: Subscriptions scraped", info.subscriptions);
    ...
    async function addProfileInfo(commentElement) {
        ...
        try {
            ...
            if (profileInfo.recentCommentsLabel && profileInfo.recentComments.length > 0) {
                const recentCommentsBox = createRecentCommentsBox(profileInfo.recentCommentsLabel, profileInfo.recentComments);
                if (recentCommentsBox) rightColumn.appendChild(recentCommentsBox);
            }
            if (profileInfo.subscriptionsLabel && profileInfo.subscriptions.length > 0) {
                const subscriptionsBox = createSubscriptionsBox(profileInfo.subscriptionsLabel, profileInfo.subscriptions);
    ...
    function processExistingComments() {
        ...
        document.querySelectorAll('ytd-comment-view-model').forEach(queueCommentForProcessing);
    }
            ...
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach(node => {
                    ...
                    if (node.tagName === 'YTD-COMMENT-VIEW-MODEL') {
                        ...
                        node.querySelectorAll('ytd-comment-view-model').forEach(comment => {
